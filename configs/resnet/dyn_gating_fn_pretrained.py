...
optimizer = dict(type='Adam',         # Optimizer type
                lr=0.01, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.0001, amsgrad=False) # Weight decay of SGD
...
lr_config = dict(policy='step',          # The policy of scheduler, also support CosineAnnealing, Cyclic, etc. Refer to details of supported LrUpdater from https://github.com/open-mmlab/mmcv/blob/master/mmcv/runner/hooks/lr_updater.py#L9.
                 step=[40, 80, 120])      # Steps to decay the learning rate
# lr_config = dict(
...
runner = dict(type='EpochBasedRunner',   # Type of runner to use (i.e. IterBasedRunner or EpochBasedRunner)
            max_epochs=150)    # Runner that runs the workflow in total max_epochs. For IterBasedRunner use `max_iters`
...
# work_dir = 'dyn_gf2_2_100'          # Directory to save the model checkpoints and logs for the current experiments.
load_from = None
work_dir = 'dyn_softgate2_eps_05'